...
app.listen(PORT, () => {
  ...
  iniciarPrecalentamiento();
});

// --- Precalentamiento de caché ---------------------------------------------
// En vez de esperar a que alguien abra Nuvio para empezar a scrapear y
// resolver IDs (lo que causaba la espera de minutos), el propio
// servidor va rellenando la caché de "novedades" en segundo plano,
// nada más arrancar y luego cada pocos minutos. Así, cuando de verdad
// llega una petición real, casi siempre ya está todo en caché y la
// respuesta es prácticamente instantánea.
//
// Nota importante: esto solo ayuda mientras el servidor está despierto.
// En el plan gratuito de Render, el servicio se "duerme" tras un rato
// sin tráfico, y la PRIMERA petición que lo despierta sigue tardando
// (arranque en frío de Render, ~30-60s, esto no lo controla el código).
// Si quieres que esa primera vez también sea instantánea, hace falta
// un "ping" externo cada pocos minutos para que Render no lo duerma
// (ver README) — o pasar a un plan de pago sin sleep.
const INTERVALO_PRECALENTAMIENTO_MS = Math.max(
  5,
  (parseInt(process.env.CACHE_TTL_MINUTES, 10) || 45) - 5
) * 60 * 1000;

async function precalentarTodo() {
  for (const platform of PLATFORMS) {
    for (const contentType of ['movies', 'series']) {
      try {
        await construirCatalogo({ platformId: platform.id, contentType, dias: DEFAULT_CONFIG.days });
      } catch (err) {
        console.error(`[precalentamiento] Fallo en ${platform.name} (${contentType}):`, err.message);
      }
    }
  }
  console.log('[precalentamiento] Ciclo completo.');
}

function iniciarPrecalentamiento() {
  precalentarTodo(); // Primera pasada nada más arrancar, sin esperar al intervalo.
  setInterval(precalentarTodo, INTERVALO_PRECALENTAMIENTO_MS);
}
