...
/**
 * Parser del CATÁLOGO COMPLETO de una plataforma (no "nuevo" — esto es
 * la ficha general del proveedor, ordenada por popularidad, paginada
 * con ?page=N). Mismo patrón de extracción a[href]+img[alt] que ya
 * conocemos, pero sin fechas: aquí no existen, JustWatch no las da
 * para el catálogo histórico completo.
 */
function parsearPaginaCatalogoCompleto(html) {
  const $ = cheerio.load(html);
  const resultados = [];

  $('a[href*="/pelicula/"], a[href*="/serie/"]').each((_, el) => {
    const $el = $(el);
    const href = ($el.attr('href') || '').replace(/^https?:\/\/[^/]+/, ''); // normaliza absolutas a relativas
    if (!/\/(pelicula|serie)\//.test(href)) return;

    const alt = ($el.find('img[alt]').first().attr('alt') || '').trim();
    if (!alt) return; // Descarta el carrusel destacado sin alt (confirmado en datos reales)

    const posterUrl = $el.find('img[alt]').first().attr('src') || null;
    const tipoDetectado = href.includes('/serie/') ? 'series' : 'movie';
    const jwSlugTitulo = href.replace(/^\/es\//, '').replace(/\//g, '_');

    resultados.push({ titulo: alt, jwSlugTitulo, jwHref: href, posterUrl, tipo: tipoDetectado });
  });

  // Quita duplicados dentro de la misma página (confirmado en datos
  // reales: "La última casa" apareció dos veces en la misma página).
  const vistos = new Set();
  return resultados.filter((r) => {
    if (vistos.has(r.jwHref)) return false;
    vistos.add(r.jwHref);
    return true;
  });
}
...
/**
 * Descarga una página concreta (1-indexed) del catálogo completo de
 * una plataforma, ya parseada.
 */
async function obtenerPaginaCatalogoCompleto(slug, tipo, pagina) {
  const jwTipo = tipo === 'series' ? 'series' : 'peliculas';
  const sufijoPagina = pagina > 1 ? `?page=${pagina}` : '';
  const url = `https://www.justwatch.com/es/proveedor/${slug}/${jwTipo}${sufijoPagina}`;
  const res = await axios.get(url, { headers: HEADERS, timeout: REQUEST_TIMEOUT_MS });
  return parsearPaginaCatalogoCompleto(res.data);
}
